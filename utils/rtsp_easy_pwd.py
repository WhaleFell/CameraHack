...
def get_rtsp(ip: str, save_path: Path) -> Tuple[bool, str]:
    """验证普通弱密码"""
    cmd = f"ffmpeg -i rtsp://admin:12345@{ip}:554 -y -f mjpeg -t 0.001 -s 1280x720 {save_path.joinpath(ip+'.jpg')}"
    cmd2 = f"ffmpeg -i rtsp://admin:admin@{ip}:554 -y -f mjpeg -t 0.001 -s 1280x720 {save_path.joinpath(ip+'.jpg')}"
